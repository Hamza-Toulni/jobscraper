...
SOURCES = [
    ...
    {
        "company": "HR Rail",
        "url": "https://jobs.infrabel.be/HRRail/go/Tous-les-vacatures-HR-Rail/957302/",
        "domain": "jobs.infrabel.be",
        "type": "successfactors",
        "job_url_regex": r"/job/[^/]+/\d+(?:-[a-z_]+)?/?$",
    },
...
def discover(source):
    if source["company"] in {"Cegeka", "Akkodis", "Infrabel", "HR Rail", "Sopra Steria"}:
...
def run():
    ...
    print()
    print("=" * 50)
    print("JOB SEARCHING AGENT V4.3 - DISCOVERY RECOVERY")
    print("=" * 50)
    ...
    for company, targets, ok_count, rejected_count in source_health:
        ...
        if diagnostic:
            ...
            print(
                ...
                f"targets={diagnostic['targets']:<3} | extracted={extracted:<3} | "
    ...
    # V4.1 stronger de-duplication. Some ATS/search paths expose the same
    # vacancy through multiple URLs. Prefer the highest-scoring representation.
    df["_dedup_company"] = df["company"].fillna("").str.lower().str.strip()
    df["_dedup_title"] = (
    ...
    df.drop(
        columns=["_dedup_company", "_dedup_title", "_dedup_location"],
        inplace=True,
    )
