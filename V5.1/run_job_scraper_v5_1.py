...
# ============================================================
# V5.1 LOCATION RESOLUTION + COUNTRY VALIDATION
# ============================================================
...
def classify_country_v50(location="", evidence="", url=""):
    """
    V5.1 conservative country classification.
    Explicit vacancy-location evidence is evaluated before generic site/URL hints.
    """
    loc = clean(location)
    ev = clean(evidence)
    url_raw = clean(url)
    strong = f"{loc} | {ev}"
    strong_low = strong.lower()

    # Strong explicit foreign city evidence.
    for city, country in FOREIGN_CITY_MARKERS.items():
        if city in strong_low:
            return "OUTSIDE_BELGIUM", country

    # Strong explicit foreign country code in location/evidence.
    for code, country in FOREIGN_COUNTRY_MARKERS.items():
        if re.search(rf"(?i)(?:,\s*|\b){re.escape(code)}(?:\b|,)", strong):
            return "OUTSIDE_BELGIUM", country

    # Explicit Belgian vacancy evidence.
    if (
        re.search(r"(?i)(?:,\s*|\b)BE(?:\b|,)", strong)
        or "belgium" in strong_low
        or "belgië" in strong_low
        or "belgique" in strong_low
    ):
        return "BELGIUM", "Belgium"

    # URL may provide useful evidence, but do not interpret a generic company
    # locale path as proof when no vacancy-location evidence exists.
    url_low = url_raw.lower()
    for city, country in FOREIGN_CITY_MARKERS.items():
        if city in url_low:
            return "OUTSIDE_BELGIUM", country

    # Encoded ATS job URLs often contain "-PL", "-NL", etc.
    for code, country in FOREIGN_COUNTRY_MARKERS.items():
        if re.search(rf"(?i)(?:-|%2c|/){re.escape(code)}(?:-|/|%2f|\b)", url_raw):
            return "OUTSIDE_BELGIUM", country
...
# ============================================================
# V5.1 BRUSSELS 30 KM LOCATION INTELLIGENCE
# ============================================================
...
# ============================================================
# V5.1 VACANCY IDENTITY / CONSERVATIVE DEDUPLICATION
# ============================================================
...
def conservative_deduplicate_dataframe(df):
    ...
    print(
        f"V5.1 conservative dedup removed {removed} proven duplicate representation(s)."
    )
    print(
...
def run():
    ...
    print()
    print("=" * 50)
    print("JOB SEARCHING AGENT V5.1 - LOCATION QUALITY CONSOLIDATION")
    print("=" * 50)
    ...
    print()
    print("=" * 50)
    print("V5.1 LOCATION INTELLIGENCE")
    print("=" * 50)
    ...
    print()
    print("=" * 50)
    print("V5.1 LOCATION REJECTIONS BY COMPANY")
    print("=" * 50)
    ...
    print()
    print("=" * 50)
    print("V5.1 LOCATION RESOLUTION BY COMPANY")
    print("=" * 50)
    ...
    if not unresolved.empty:
        print()
        print("=" * 50)
        print("V5.1 UNRESOLVED LOCATION DIAGNOSTICS")
        print("=" * 50)
