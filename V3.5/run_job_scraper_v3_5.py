...
def discover_capgemini(source):
    print("  strategy: Capgemini Belgium listing + careers portal + live validation")

    candidates = []

    # 1) Official Belgium search page. It exposes current Belgium vacancy
    # titles even though the detail links may point to careers.capgemini.com.
    try:
        html = fetch(source["url"])
        soup = BeautifulSoup(html, "html.parser")
        listing_links = 0
        ...
        for anchor in soup.find_all("a", href=True):
            ...
            if not (
                host == "capgemini.com"
                or host.endswith(".capgemini.com")
            ):
                continue
        ...
        print("  official listing job links:", listing_links)

    except Exception as exc:
        print("  Capgemini Belgium page skipped:", exc)

    # 2) Search the actual Capgemini careers host. This is where current
    # individual Belgian job-detail pages are published.
    queries = [
        'site:careers.capgemini.com/job/ Diegem "Data Analyst"',
        'site:careers.capgemini.com/job/ Diegem "BI Engineer"',
        'site:careers.capgemini.com/job/ Diegem "Data Engineer"',
        'site:careers.capgemini.com/job/ Diegem "Power BI"',
        'site:careers.capgemini.com/job/ Diegem "Data Governance"',
        'site:careers.capgemini.com/job/ Diegem "Data Quality"',
        'site:careers.capgemini.com/job/ Diegem "Data strategist"',
    ]
    ...
    for query in queries:
        ...
        try:
            ...
            print("  Capgemini DuckDuckGo discovery skipped:", exc)
        ...
        try:
            ...
            print("  Capgemini Bing discovery skipped:", exc)

        for url, title in discovered:
            if not is_job_url(url, source):
                continue

            path_text = urlparse(url).path.replace("-", " ")
            if "diegem" not in path_text.lower() and "belgium" not in title.lower():
                continue

            if looks_targeted(clean(title + " " + path_text)):
                candidates.append((url, title))
                indexed_found += 1
    ...
    # 3) Verified current Belgian vacancy pages. These are a fallback only:
    # the normal detail-page parser and hard filters still decide whether
    # each vacancy is real, targeted and suitable.
    live_seeds = [
        (
            "https://careers.capgemini.com/job/Diegem-BI-Engineer-Data-Analyst/1430484633/",
            "BI Engineer | Data Analyst",
        ),
        (
            "https://careers.capgemini.com/job/Diegem-Junior-BI-EngineerData-Analyst/1431392133/",
            "BI Engineer/Data Analyst (Experienced Junior)",
        ),
        (
            "https://careers.capgemini.com/job/Diegem-Data-Engineer-Modern-Data-Platforms/1378947233/",
            "Data Engineer Modern Data Platforms",
        ),
        (
            "https://careers.capgemini.com/job/Diegem-Senior-Databricks-Data-Engineer/1391304533/",
            "Senior Databricks Data Engineer",
        ),
        (
            "https://careers.capgemini.com/job/Diegem-BE-EDA-Data-strategist/1405840133/",
            "BE - EDA - Data strategist",
        ),
    ]
...
def discover_akkodis(source):
    print("  strategy: Akkodis Belgium indexed discovery + live validation")

    candidates = []

    # 1) Try current Belgian Akkodis entry pages. These pages may be
    # client-rendered, so direct HTML discovery is only the first layer.
    entry_pages = [
    ...
    print("  direct target candidates:", direct_found)

    # 2) Indexed discovery. Akkodis individual Belgian vacancy pages currently
    # use /en-be/careers/jobs/<slug>/<reference>.
    queries = [
        ...
        'site:akkodis.com/en-be/careers/jobs/ "BI Developer"',
        ...
        'site:akkodis.com/en-be/careers/jobs/ "People Analytics"',
        'site:akkodis.com/en-be/careers/jobs/ "HR Data"',
        'site:akkodis.com/en-be/careers/jobs/ "Functional Data Analyst"',
    ]
    ...
    for query in queries:
        ...
        try:
            ...
            print("  Akkodis DuckDuckGo discovery skipped:", exc)
        ...
        try:
            ...
            print("  Akkodis Bing discovery skipped:", exc)
    ...
    # 3) Live validation seeds. These are safety-net candidates, not static
    # job records: scrape_source() still fetches each page and applies normal
    # title-family detection, vacancy extraction and hard filters. Removed or
    # stale URLs therefore disappear naturally.
    live_seeds = [
        (
            "https://www.akkodis.com/en-be/careers/jobs/"
            "data-engineer/2026-34883",
            "Data Engineer",
        ),
    ]
...
def run():
    ...
    print()
    print("=" * 50)
    print("JOB SCRAPER V3.5 - AKKODIS DISCOVERY VERSION")
    print("=" * 50)
