...
# ============================================================
# V5.3 LOCATION RESOLUTION + COUNTRY VALIDATION
# ============================================================
...
def classify_country_v50(location="", evidence="", url=""):
    """
    V5.3 country classification.

    Two-letter country codes are trusted only in bounded/structured location
    fields. They are never searched for in arbitrary vacancy-description text.
    """
    loc = clean(location)
    url_raw = clean(url)
    ...
    bounded = " | ".join([loc] + structured_raw)
    bounded_low = bounded.lower()

    # A recognized Belgian city/postcode is strong Belgian evidence.
    ...
    # BE is safe only in bounded location-like fields.
    if re.search(r"(?i)(?:^|[\s,;/|()\-])BE(?:$|[\s,;/|()\-])", bounded):
        return "BELGIUM", "Belgium"

    # Full foreign country names.
    foreign_names = {
        country.lower(): country
        for country in set(FOREIGN_COUNTRY_MARKERS.values())
    }
    ...
    # Explicit foreign cities.
    for city, country in FOREIGN_CITY_MARKERS.items():
        if re.search(r"(?<![\w-])" + re.escape(city) + r"(?![\w-])", bounded_low, re.I):
            return "OUTSIDE_BELGIUM", country

    # Two-letter foreign codes ONLY in bounded location-like evidence.
    for code, country in FOREIGN_COUNTRY_MARKERS.items():
        if re.search(
            rf"(?i)(?:^|[\s,;/|()\-]){re.escape(code)}(?:$|[\s,;/|()\-])",
            bounded
        ):
            return "OUTSIDE_BELGIUM", country

    # URL evidence is secondary and must be explicit.
    for city, country in FOREIGN_CITY_MARKERS.items():
        if city in url_low:
            return "OUTSIDE_BELGIUM", country
...
# ============================================================
# V5.3 BRUSSELS 30 KM LOCATION INTELLIGENCE
# ============================================================
...
# ============================================================
# V5.3 VACANCY IDENTITY / CONSERVATIVE DEDUPLICATION
# ============================================================
...
def conservative_deduplicate_dataframe(df):
    ...
    print(
        f"V5.3 conservative dedup removed {removed} proven duplicate representation(s)."
    )
    print(
...
def run():
    ...
    print()
    print("=" * 50)
    print("JOB SEARCHING AGENT V5.3 - SOURCE RECOVERY + RELIABILITY")
    print("=" * 50)
    ...
    print()
    print("=" * 50)
    print("V5.3 SOURCE REJECTION RECONCILIATION")
    print("=" * 50)
    ...
    print()
    print("=" * 50)
    print("V5.3 SOURCE RECOVERY DIAGNOSTICS")
    print("=" * 50)
    for dead_name in ("Akkodis", "Infrabel", "HR Rail"):
        stat = source_stats.get(dead_name, {})
        scanned = stat.get("scanned", stat.get("targets", 0))
        extracted = stat.get("extracted", stat.get("targets", 0))
    ...
    print()
    print("=" * 50)
    print("V5.3 LOCATION INTELLIGENCE")
    print("=" * 50)
    ...
    print("REPORT-ONLY: OUTSIDE_30KM jobs are NOT rejected in V5.1.")

    print()
    print("=" * 50)
    print("V5.3 LOCATION REJECTIONS BY COMPANY")
    print("=" * 50)
    if location_rejections_v51:
        for company, count in sorted(location_rejections_v51.items()):
    ...
    print()
    print("=" * 50)
    print("V5.3 LOCATION RESOLUTION BY COMPANY")
    print("=" * 50)
    ...
    if not unresolved.empty:
        print()
        print("=" * 50)
        print("V5.3 UNRESOLVED LOCATION DIAGNOSTICS")
        print("=" * 50)
