...
# ============================================================
# V4.5 VACANCY IDENTITY / SMART DEDUPLICATION
# ============================================================

def vacancy_reference(company, url):
    """
    Return a stable vacancy reference when the ATS exposes one in the URL.

    The reference is deliberately company-aware. We only collapse records
    across different URLs when we have strong evidence that they represent
    the same underlying vacancy.
    """
    company = clean(company).lower()
    path = urlparse(clean(url)).path.rstrip("/")

    patterns = {
        # EN and NL Cegeka pages can expose the same vacancy under different
        # translated slugs while retaining the same numeric vacancy ID.
        "cegeka": r"-(\d+)$",

        # Sopra Steria exposes an explicit JID.
        "sopra steria": r"-jid-(\d+)$",

        # Teamtailor/Keyrus job URLs start with a numeric vacancy ID.
        "keyrus": r"/jobs/(\d+)-",

        # SuccessFactors-style detail URLs commonly end in a numeric ID.
        "infrabel": r"/(\d+)(?:-[A-Za-z_]+)?$",
        "hr rail": r"/(\d+)(?:-[A-Za-z_]+)?$",
    }

    pattern = patterns.get(company)
    if not pattern:
        return ""
...
def normalized_title_for_dedup(title):
    value = clean(title).lower()

    # Remove common seniority/noise markers only for the *fallback*
    # comparison. We do NOT merge jobs solely because this normalized title
    # matches; location must also match and no stable vacancy ID may conflict.
    value = re.sub(
        r"\b(m/f/x|m/v/x|f/m/x|h/f/x|m/w/d)\b",
        " ",
        value,
        flags=re.I,
    )
...
def smart_deduplicate_dataframe(df):
    """
    V4.5 two-stage deduplication.

    Stage 1: company + stable vacancy reference.
      This removes multilingual/alternate URL representations safely.

    Stage 2: company + normalized title + normalized location, but only for
      rows without a stable reference. This preserves genuinely separate jobs
      when their ATS exposes different vacancy IDs.
    """
    if df.empty:
        return df
    ...
    work["_dedup_company"] = (
        work["company"].fillna("").astype(str).str.lower().str.strip()
    )
    work["_job_reference"] = [
        vacancy_reference(company, url)
        for company, url in zip(work["company"], work["url"])
    ]
    work["_dedup_title"] = work["title"].fillna("").map(
        normalized_title_for_dedup
    )
    work["_dedup_location"] = (
        work["location"].fillna("").astype(str).str.lower()
    ...
    if not with_ref.empty:
        with_ref.drop_duplicates(
            subset=["_dedup_company", "_job_reference"],
            keep="first",
            inplace=True,
        )
    ...
    if not without_ref.empty:
        without_ref.drop_duplicates(
            subset=["_dedup_company", "_dedup_title", "_dedup_location"],
            keep="first",
            inplace=True,
        )
    ...
    print(
        f"Smart deduplication removed {removed} duplicate representation(s)."
    )
...
def run():
    ...
    print()
    print("=" * 50)
    print("JOB SEARCHING AGENT V4.5 - SMART DEDUP + EXPANSION")
    print("=" * 50)
    ...
    # V4.5 smart de-duplication:
    # prefer stable ATS/job references; use title+location only when no
    # reliable vacancy reference exists.
    df = smart_deduplicate_dataframe(df)
