...
def extract_jsonld_job(html, source, page_url):
    ...
    for obj in jsonld_objects(soup):
        ...
        if not location:
            location, _ = resolve_location_v48(
                soup,
                description,
                existing="",
            )
...
def resolve_location_v48(soup, description="", existing=""):
    """
    V4.8 hierarchy:
      1. already extracted structured location
      2. ATS/location DOM selectors
      3. location/address metadata
      4. data attributes / serialized application state
      5. labelled location snippets
      6. conservative vacancy-body municipality/postcode scan
    """
    existing = clean(existing)
    if existing:
        resolved = _location_from_text(existing)
        return resolved or existing, "existing"
    ...
    resolved = _meta_location(soup)
    if resolved:
        return resolved, "meta"

    resolved = _structured_location(soup)
    if resolved:
        return resolved, "structured"

    page_text = clean(soup.get_text(" ", strip=True))
    for match in re.finditer(
        r"(?i)\b(?:location|locatie|lieu|standplaats|werkplaats|workplace)"
        r"\s*[:\-]\s*([^|•;\n]{2,100})",
        page_text,
    ):
        resolved = _location_from_text(match.group(1))
...
def extract_detail_job(
    ...
    location, location_source = resolve_location_v48(
        soup,
        description,
...
# ============================================================
# V4.8 BRUSSELS 30 KM LOCATION INTELLIGENCE
# ============================================================
...
# ============================================================
# V4.8 VACANCY IDENTITY / CONSERVATIVE DEDUPLICATION
# ============================================================
...
def conservative_deduplicate_dataframe(df):
    ...
    print(
        f"V4.8 conservative dedup removed {removed} proven duplicate representation(s)."
    )
    print(
...
def run():
    ...
    print()
    print("=" * 50)
    print("JOB SEARCHING AGENT V4.8 - LOCATION RESOLUTION UPGRADE - BRUSSELS 30KM LOCATION INTELLIGENCE")
    print("=" * 50)
    ...
    print()
    print("=" * 50)
    print("V4.8 LOCATION INTELLIGENCE")
    print("=" * 50)
    ...
    print("REPORT-ONLY: OUTSIDE_30KM jobs are NOT rejected in V4.8.")
